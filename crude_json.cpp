...
std::pair<value, bool> value::load(const string& path) {
    ...
    FILE* handle = nullptr;
    if (fopen_s(&handle, path.c_str(), "rb") != 0) return {value{}, false};
    file.reset(handle);
#else
...
bool value::save(const string& path, const int indent,
...
#if defined(_MSC_VER) || (defined(__STDC_LIB_EXT1__) && __STDC_WANT_LIB_EXT1__)
    FILE* handle = nullptr;
    if (fopen_s(&handle, path.c_str(), "wb") != 0) return false;
    file.reset(handle);
#else
    ...
    if (fwrite(data.data(), data.size(), 1, file.get()) != 1) return false;
