...
# undef CRUDE_MAX2
    struct storage_t {
        alignas(max_align) std::byte data[max_size];
    };
    static_assert(sizeof(storage_t) == max_size, "Storage size mismatch");
    static_assert(alignof(storage_t) == max_align, "Storage alignment mismatch");
