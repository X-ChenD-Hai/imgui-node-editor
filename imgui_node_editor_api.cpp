...
bool ax::NodeEditor::Link(LinkId id, PinId startPinId, PinId endPinId,
    ...
    return s_Editor->DoLink(id, startPinId, endPinId, ImColor(color),
                            thickness);
}

void ax::NodeEditor::Flow(LinkId linkId, FlowDirection direction) {
    if (auto link = s_Editor->FindLink(linkId)) s_Editor->Flow(link, direction);
}
...
bool ax::NodeEditor::GetLinkPins(LinkId linkId, PinId* startPinId,
    ...
    if (startPinId) *startPinId = link->m_StartPin->m_ID;
    if (endPinId) *endPinId = link->m_EndPin->m_ID;
